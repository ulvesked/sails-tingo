  ...
  var adapter = {
      ...
      new Connection(connection, function(_err, db) {
        ...
        if(_err) {
          return cb((function _createError(){
            var msg = util.format('Failed to connect to TingoDB. Are you sure dbPath is properly configured in config/connections.js?\n Error details:\n%s', util.inspect(_err, false, null));
    ...
    describe: function(connectionName, collectionName, cb) {
      ...
      connectionObject.connection.db.collectionNames(collectionName, function(err, names) {
        ...
        if(names && names.length > 0) return cb(null, schema);
        cb();
      });
    },
     ...
     * Define
     *
     * Create a new TingoDB Collection and set Index Values
     *
     * @param {String} connectionName
     * @param {String} collectionName
     ...
     * Native
     *
     * Give access to a native tingo collection object for running custom
     * queries.
     *
     * @param {String} connectionName
     * @param {String} collectionName
    ...
    /**
     * TingoDB object with TingoDB native methods
     */
    ...
    tingo: {
       ...
       * ObjectId
       *
       * Return a TingoDB ObjectID from a string
       *
       * @param {String} id
       */
     ...
     * Join
     *
     * Peforms a join between 2-3 tingo collections when Waterline core
