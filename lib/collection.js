...
Collection.prototype.update = function update(criteria, values, cb) {
  ...
  // TingoDB doesn't allow ID's to be updated
...
Collection.prototype._parseDefinition = function _parseDefinition(definition) {
  ...
  // Remove any Auto-Increment Keys, TingoDB currently doesn't handle this well without
...
Collection.prototype._buildIndexes = function _buildIndexes() {
  ...
  Object.keys(this.schema).forEach(function(key) {
    ...
    // If index key is `id` ignore it because TingoDB will automatically handle this
    if(key === 'id') {
      return;
    }
